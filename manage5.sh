...
function importContent {
    cp "$src/api/v5/canonical/strings/$x" "content/$x"
}
...
if [ "$choice" = "i" ]; then
	...
	cp -rf build/content/* $web/api/v5/content/
	for D in $web/api/v5/content/*/; do
		cp -r $web/api/v5/canonical/css $D/
		cp $web/api/v5/canonical/defaults/* $D/
		cp -n $web/api/v5/canonical/strings/* $D/
	done

	# English is not exported by default
	rm -rf $web/api/v5/content/en_US
	...
	cp -rf build/content/* $web/api/v5/content_dns/
	for D in $web/api/v5/content_dns/*/; do
		cp -r $web/api/v5/canonical/css $D/
		cp $web/api/v5/canonical/defaults/* $D/
		cp -n $web/api/v5/canonical/strings/* $D/
	...
	cp -rf build/content/en_US/*.html $web/api/v5/canonical/strings/
